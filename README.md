...
- Функциональный подход: fp-ts используется для безопасной обработки ошибок в API-слое (монады TaskEither, Option, Task).
...
- Нет девелопмент сборки докер контейнера с девсервером. По сути, там mvp продакшн сборки.
  Если есть желание, чтобы девсервер вите (и хотрелуод) работали в докере, нужно
  добавить специальные настройки в vite cofnig (server, host, polling) и создать отдельный
  докерфайл для дева (или апгрейдить docker compose file).
- Некоторые архитектурные улучшения: возможно, было бы неплохо что-то вынести в widgets, хотя
  лично мне с FSD не всегда очевидно, как лучше поступить. В целом показалось, что
  ввиду относительно маленького проекта такое жесткое дробление не требуется, но products по
  итогу вышла относительно большой фичей. По итогу однозначного мнения по этому поводу у меня
  не сложилось и в итоге я оставил так, как есть.
